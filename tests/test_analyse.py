...
Ces fonctions decident des seuils publies. Une erreur ici ne fait pas planter un test, elle
publie un chiffre faux avec l'air d'avoir ete mesure, ce qui est pire.
"""
import math

from grille.analyser import (BUDGET_FP, GRAINE_VALIDATION, courbe, mesurer, point_retenu,
                             separer, wilson)
...
def test_la_courbe_va_du_tout_au_rien():
    pts = courbe(pos=[3.0, 4.0, 5.0], neg=[0.0, 1.0, 2.0])
...
def test_une_separation_parfaite_donne_un_point_parfait():
    pts = courbe(pos=[10.0, 11.0], neg=[0.0, 1.0])
...
def test_le_point_retenu_respecte_le_budget_de_faux_positifs():
    ...
    pt = point_retenu(courbe(pos, neg), budget=0.05)
...
def test_un_budget_a_zero_rend_un_rappel_nul_et_pas_none():
    """Il existe toujours un seuil a zero faux positif: celui qui ne declenche jamais.

    Ce n'est donc jamais l'absence de point qui signale un controle inutilisable, c'est un
    rappel trop bas au point retenu. Confondre les deux ferait croire qu'un controle est
    mesure alors qu'il est simplement muet.
    """
    pt = point_retenu(courbe(pos=[1.0], neg=[2.0]), budget=0.0)
    assert pt is not None and pt["rappel"] == 0.0 and pt["fpr"] == 0.0


def test_pas_de_mesure_pas_de_point():
    assert point_retenu([]) is None


def test_la_precision_depend_de_la_prevalence():
    pts = courbe(pos=[10.0] * 100, neg=[0.0] * 99 + [10.0])
...
def test_la_separation_isole_la_graine_de_validation():
    par_cellule = {(0.0, 200, 95, 0.0, g): (1.0, 0.0) for g in (11, 23, GRAINE_VALIDATION)}
    cal, val = separer(par_cellule)
    assert len(cal) == 2 and len(val) == 1
    assert all(k[4] != GRAINE_VALIDATION for k in cal)


def test_mesurer_compte_ce_qui_depasse_strictement_le_seuil():
    m = mesurer({(0.0, 200, 95, 0.0, 11): {"pos": {"a": 5.0}, "neg": {"a": 5.0, "b": 1.0}},
...
def test_le_taux_par_dossier_est_bien_plus_haut_que_le_taux_par_cible():
    """C'est le piege du chiffre annonce: dix-sept champs a 1% chacun ne font pas 1% de
    dossiers propres. Un dossier crie des qu'UNE de ses cibles crie."""
    cellules = {}
    for i in range(100):
        neg = {f"champ{j}": (9.0 if (i % 10 == j) else 0.0) for j in range(10)}
        cellules[(0.0, 200, 95, 0.0, i)] = {"pos": {"champ0": 9.0}, "neg": neg}
    m = mesurer(cellules, seuil=5.0)
    assert math.isclose(m["fpr"], 0.10), "une cible sur dix se declenche"
