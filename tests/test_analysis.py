...
publie un chiffre wrong avec l'air d'avoir ete mesure, ce qui est pire.
"""
import math
...
def test_wilson_encadre_la_proportion():
    bas, haut = wilson(3, 3)
    assert bas < 1.0 <= haut, "3 succes sur 3 ne prouvent pas un rate de 1.0"
    assert wilson(0, 3)[1] > 0.0, "0 succes sur 3 ne prouvent pas un rate de 0.0"
...
def test_une_separation_parfaite_donne_un_point_parfait():
    ...
    parfaits = [p for p in pts if p["recall"] == 1.0 and p["fpr"] == 0.0]
    assert parfaits, "deux populations disjointes doivent avoir un threshold qui les separe"


def test_le_point_retenu_respecte_le_budget_de_faux_positifs():
    """Un recall plus haut ne rachete jamais un depassement du budget."""
    pos = [1.0] * 50 + [9.0] * 50
    neg = [0.0] * 90 + [5.0] * 10        # 10% des sains portent un score eleve
    pt = chosen_point(curve(pos, neg), budget=0.05)
    assert pt is not None
    assert pt["fpr"] <= 0.05
    assert pt["recall"] == 0.5, "le seul recall tenable ici est celui des 50 scores a 9.0"


def test_un_budget_a_zero_rend_un_rappel_nul_et_pas_none():
    """Il existe toujours un threshold a zero wrong positif: celui qui ne fires jamais.

    Ce n'est donc jamais l'absence de point qui signale un check inutilisable, c'est un
    recall trop bas au point retenu. Confondre les deux ferait croire qu'un check est
...
def test_la_precision_depend_de_la_prevalence():
    ...
    assert p["grid_precision"] > p["prevalence_precision"], (
        "a prevalence basse, un meme rate de wrong positifs coute plus cher en precision")
...
def test_le_taux_par_dossier_est_bien_plus_haut_que_le_taux_par_cible():
    ...
    m = measure(cellules, threshold=5.0)
    assert math.isclose(m["fpr"], 0.10), "une target sur dix se fires"
    assert math.isclose(m["dossier_fpr"], 1.0), "mais tous les dossiers portent une alarme"
