...
du voisin est un wrong positif, et un wrong positif coute la credibilite de toute la gate: une
...
#     mesuree sur low_resolution: 0/864 dossiers du domain.
...
# ou la grid le mesure pourtant a 1,000 de recall. La tolerance est donc le bon endroit
# pour porter ce reste, et son chiffre est dans LIMITES.md.
#
# La cell des fixtures est l'une des 270 sur 288 ou ce declenchement ne se produced pas: le
...
def declenches(readings, reference, name, clock="filing"):
    """Aucun settings n'est passe: on veut le path de PRODUCTION, celui qui lit thresholds.json.

    Un test qui passerait Settings() court-circuiterait les sensors et les thresholds mesures par
    la grid et testerait les values de spike, c'est-a-dire pas le produced.
    """
...
def test_chaque_controle_est_couvert_par_une_variante():
    """Un check sans variant n'a ni vrai positif ni wrong negatif mesurable."""
