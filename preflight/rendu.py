...
def _cle(pdf, dpi, page):
    st = os.stat(pdf)
    brut = f"{os.path.abspath(pdf)}|{st.st_mtime_ns}|{st.st_size}|{dpi}|{page}"
    return hashlib.sha1(brut.encode()).hexdigest()[:16]


def rendre(pdf, dpi=CANON_DPI, page=1, cache=None):
    """Page rendue en niveaux de gris, uint8. Retour: tableau numpy (H, W)."""
    rep = cache or CACHE
    os.makedirs(rep, exist_ok=True)
    dest = os.path.join(rep, f"{_cle(pdf, dpi, page)}.png")
