#!/usr/bin/env python3
"""Premier test vert. Chaine complete sur un formulaire public blank, zero data reelle.

    formulaire blank -> rempli a moitie (person inventee) -> render -> degrade
    -> redresse -> trois sensors -> constats

TROIS CAPTEURS, ET C'EST LE RESULTAT PRINCIPAL DE CE SPIKE. Il a fallu trois passes:

  1. l'ink brute dans la zone a annonce 8 boxes cochees alors que rien n'etait coche.
     Cause: le scan est tourne et il etait compare a une reference droite.
     -> TOUT check par coordonnees exige un REDRESSEMENT d'abord.
  2. apres deskew les boxes sont exactes, mais un field texte VIDE lisait encore
     +2,44% d'ink (contre +4,5 a +5,1 pour un field rempli): separable, mais fragile.
     -> l'ink est le mauvais capteur pour du TEXTE.
  3. positions de words OCR pour le texte, ink differentielle pour les boxes: net.

Le formulaire blank n'est pas qu'une fixture, c'est l'IMAGE DE REFERENCE: chaque check
est un differentiel contre lui, et les words pre-imprimes lus sur le blank sont soustraits,
...
W-9), donc aucune coordonnee ne se mesure a la main.
    ...
    python3 spike/proof.py     # exit 0 si les trois sensors sont d'accord
"""
...
BLANK_PDF = os.path.normpath(os.path.join(ICI, "..", "corpus", "fw9.pdf"))
DPI, MARGE, SEED = 200, 6, 7
SKEW, SIGMA, FLOU, JPEG = 0.45, 6, 0.4, 55

FICTIONAL = {"f1_01[0]": "MARISOL QUISPE VARGAS",     # person inventee
          "f1_07[0]": "128 RUE DES ACACIAS",
          "f1_08[0]": "COMMENTRY, FR 03600",
          "c1_1[0]":  "/1"}                         # une seule case cochee
DELIBERATELY_EMPTY = "f1_11[0]"                            # defaut injecte: number tax jamais rempli
...
def declared_zones(pdf):
    """Le PDF fillable declare lui-meme ou sont ses fields."""
...
def main():
    tmp = os.path.join(ICI, ".task")
    os.makedirs(tmp, exist_ok=True)
    rempli = os.path.join(tmp, "rempli.pdf")
    remplir(rempli)
    png_vierge = render(BLANK_PDF, os.path.join(tmp, "blank"))
    png_propre = render(rempli, os.path.join(tmp, "propre"))
    ...
    attendu = {"f1_01[0]": True, "f1_07[0]": True, "f1_08[0]": True, DELIBERATELY_EMPTY: False}
    for k, doit_etre_rempli in attendu.items():
        x0, y0, x1, y1, _ = zones[k]
        dedans = [t for t, cx, cy in words
    ...
    # C3 boxes: ink differentielle dans le disque central
    print("\n=== C3 boxes a cocher (scan moins blank) ===")
    for k, z in sorted(zones.items()):
        if z[4] != "/Btn":
            continue
        d = disque(SCAN, z) - disque(VIERGE, z)
        cochee, doit = d > 8, (k in FICTIONAL)
        ok = cochee == doit
