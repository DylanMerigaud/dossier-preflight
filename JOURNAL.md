...
Domaine nominal: **numerisation a 150 dpi ou plus**. Les huit checks autres que les valeurs
interdites tiennent 1,000 de rappel a 0,0000 de faux positifs par target. Les valeurs
...
alarme, 0,46%**, tous du check des valeurs interdites.
...
| check | seuil | pourquoi celui-la |
|---|---|---|
| required_field | -0.345 % d'ink ajoutee | capteur d'ink, gagnant du duel a 0 faux positif contre 0,0003 pour l'OCR |
| required_checkbox | delta -14.38 | disque a 0,30 du cote, seuil d'ink 128, meilleur des 12 reglages |
...
de choisir le seuil donnait 100% de rappel sur la seule target survivante. Definir la
certifiability au rappel plein laissait un positif aberrant trainer le seuil jusqu'a 0,11 de
ressemblance. L'abstention sous le floor, branchee pendant la recherche de domaine, faisait
...
**Le seuil de expiry a failli etre fitte, et le test des deux clocks l'a attrape.** La
...
**Le spike avait tranche a n=1 dans le mauvais sens.** Il concluait que l'ink etait le
...
- **Le coin des valeurs interdites est tranche, et il est REEL.** 12 graines neuves plus les 3
  ...
  la moyenne d'un n=18) mais la borne haute reste sous le floor. Un temoin a JPEG 30 sur les
  memes graines rend 0,986, donc c'est bien la compression et pas des graines dures. Et la
  shape reelle est une conjonction de QUATRE facteurs, pas de deux: 300 dpi ET JPEG 95 ET bruit
  ...
  seuil: `python3 grid/corner_followup.py`.
...
- **Les degradations sont synthetiques.** Ni courbure de page, ni ombre de reliure, ni
  poussiere, ni moire de retramage. La grid borne le domaine, elle ne le prouve pas.
