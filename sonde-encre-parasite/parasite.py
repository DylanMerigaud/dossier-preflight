...
Le direction de l'erreur compte: un field vide declare rempli est un FAUX NEGATIF, le filing
...
def tache(grey, zone, dpi, fraction, rng, noirceur=45):
...
def pliure(grey, zone, dpi, fraction, rng):
    ...
    la width de la page, ce qui varie avec la profondeur du pli et l'exposition du scanner
    c'est a quel point elle est sombre. Et il y a une falaise: tant que l'ombre laisse le
    papier au-dessus du threshold de binarisation du capteur (128), elle est parfaitement
    ...
    profil = np.exp(-0.5 * ((ys - centre) / width) ** 2) * force
    a = grey.astype(np.float32) * (1.0 - profil[:, None])
    return np.clip(a, 0, 255).astype(np.uint8)


def trait(grey, zone, dpi, fraction, rng, noirceur=40):
    """Un trait de stylo parti d'a cote et qui deborde dans la zone."""
    x0, y0, x1, y1 = _zone_scan(zone, dpi)
    height = y1 - y0
    ep = max(1.5, height * (0.06 + 1.2 * fraction))
