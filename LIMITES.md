...
Les points de fonctionnement sont choisis sur les 864 couples de ce domain,
...
choisi comme le recall le plus haut tenant un rate de faux positifs sous 0.2%.

Le threshold est choisi sur les graines 11 et 23, et le chiffre publie est celui de la
seed 37, jamais regardee avant. Choisir un threshold et rapporter son
recall sur les memes tirages le surestime toujours.

Une cell est declaree HORS DOMAINE quand le recall y passe sous 95%.
...
Cellule: 300 dpi, JPEG 95, bruit sigma 12.0, check forbidden_value, threshold publie 0.7214.

| jeu | recall | IC 95% | positifs |
|---|---|---|---|
| graines d'origin (11, 23, 37) | 0.8333 | [0.608, 0.942] | 18 |
...
| jeu | recall | IC 95% | faux positifs par target | IC 95% | par dossier clean | positifs |
|---|---|---|---|---|---|---|
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 0.958 [0.929, 0.976], declenchements sur dossier clean 0.8403 sur 288 cibles.
...
Aucune cell du domain explore ne passe sous le floor.

Rappel par facteur, au threshold retenu:
...
Reglage retenu: aucun settings. Seuil 0.08795.
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 1.000 [0.987, 1.000], declenchements sur dossier clean 0.0000 sur 864 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 1.000 [0.566, 1.000], declenchements sur dossier clean 0.0000 sur 2 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.
...
Pires CROISEMENTS de deux factors. Une reading axe par axe peut mentir par
omission: trois values marginales toutes au-dessus du floor peuvent se
croiser en une cell qui passe dessous.

- dpi x sigma = [300, 12.0]: recall 0.931 [0.848, 0.970] sur 72 positifs
- angle x jpeg = [1.0, 95]: recall 0.944 [0.819, 0.985] sur 36 positifs
- angle x sigma = [2.0, 12.0]: recall 0.944 [0.819, 0.985] sur 36 positifs
- dpi x jpeg = [300, 95]: recall 0.944 [0.866, 0.978] sur 72 positifs
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 0.042 [0.024, 0.071], declenchements sur dossier clean 0.0000 sur 1728 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.

Frontiere par facteur, nombre de cellules fallen_cells sur le total:
...
Les pires cellules (angle, dpi, jpeg, sigma) et leur recall:

- angle 0.25, 300 dpi, JPEG 55, sigma 12.0: recall 0.67 sur 3 graines
- angle 1.0, 300 dpi, JPEG 95, sigma 6.0: recall 0.67 sur 3 graines
- angle 1.0, 300 dpi, JPEG 95, sigma 12.0: recall 0.67 sur 3 graines
- angle 2.0, 300 dpi, JPEG 75, sigma 12.0: recall 0.67 sur 3 graines
- angle 2.0, 300 dpi, JPEG 95, sigma 12.0: recall 0.67 sur 3 graines
- angle 4.0, 300 dpi, JPEG 95, sigma 12.0: recall 0.67 sur 3 graines
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 1.000 [0.987, 1.000], declenchements sur dossier clean 0.0000 sur 5760 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.
...
Reglage retenu: aucun settings. Seuil -148.5.
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 1.000 [0.987, 1.000], declenchements sur dossier clean 1.0000 sur 864 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.

Pour CE check, ces declenchements hors domain ne sont pas des faux
...
Reglage retenu: aucun settings. Seuil 0.3585.
...
Hors domain (dpi < 150), CAPTEURS BRUTS, c'est-a-dire ce que l'outil ferait
s'il n'avait pas la regle d'abstention: recall 1.000 [0.987, 1.000], declenchements sur dossier clean 0.0000 sur 288 cibles.
Ces chiffres-la ne decrivent donc pas le produit, ils justifient la regle: en
production, un check qui LIT s'abstient sous le floor au lieu de produire
ce qu'on lit ici. Ils sont mesures sensors bruts a dessein, parce qu'une mesure
ne peut pas dependre du comportement qu'elle sert a regler.
